import { OPTIONS_LIMIT } from "../config";
const WeightedList = require('js-weighted-list');

export function getOption(optionsLits: any[]): any {        
...
export function validateOptionsList(optionsLits: object[]): any[]{
    const limit:number = +String(OPTIONS_LIMIT);
    let arrOptions: object[];
...
function searchDuplicates(optionsList: any[]): any[]{
    let arr: any[]= [];
...
function validateProbabilityOption(option: any){
    if(option.probability  <0) {
        throw new Error('La probabilidad de la opcion no debe ser menor a 0.');
    }
}

function sumAllOdds(list: any[]): any[]{
    const sum: number = list.map(item => item.probability).reduce((prev,curr)=> prev+curr,0);
